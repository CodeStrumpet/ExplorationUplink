 ...
 * Currently only ImageFormat_NV21 and ImageFormat_YUY2 are supported.
 *
 * To compress a rectangle region in the YUV data, users have to specify the
 * region by left, top, width and height.
 */
public class A15C_YuvImage {

	
	private final static int ImageFormat_YUY2 = 20;
	private final static int ImageFormat_NV21 = 17;
   ...
   /**
     ...
     *                null.
     */
    public A15C_YuvImage(byte[] yuv, int format, int width, int height, int[] strides) {
        if (format != ImageFormat_NV21 &&
                format != ImageFormat_YUY2) {
            throw new IllegalArgumentException(
                    "only support ImageFormat_NV21 " +
                    "and ImageFormat_YUY2 for now");
        }
     ...
     * Only ImageFormat_NV21 and ImageFormat_YUY2
